...
// ===> SỬA LỖI CANNOT GET Ở ĐÂY <===
// 1. Phục vụ file tĩnh từ thư mục 'public' (nơi chứa index.html)
app.use(express.static(path.join(__dirname, 'public')));

// 2. Phục vụ thư mục uploads
app.use('/uploads', express.static(path.join(__dirname, 'uploads')));

// 3. Đường dẫn gốc trỏ về index.html trong public
app.get('/', (req, res) => {
...
const TruckingSchema = new mongoose.Schema({
    ...
    // Mảng chứa chi tiết từng cont (Nghiệp vụ, đi/đến, giá riêng)
    containers: [{
        vehicle: String,        
        contNo: String,         
        contType: String,       
        operation: String,      // Nghiệp vụ riêng
        origin: String,         // Nơi đi riêng
        destination: String,    // Nơi đến riêng
        cost: Number,           // Giá riêng
        extraCost: Number,      // Phụ phí riêng
        liftingCost: Number,    // Nâng hạ riêng
        emptyDepot: String      // Bãi rỗng riêng
    }],

    // Tổng hợp (để hiện thống kê nhanh)
    cost: Number,            
...
app.post('/api/trucking', cpUpload, async (req, res) => {
    try {
        ...
        if (data.containersList) {
            data.containers = JSON.parse(data.containersList);
        }

        if (req.files['invoiceFiles']) {
            req.files['invoiceFiles'].forEach(file => {
                data.invoices.push({ path: file.path.replace(/\\/g, "/"), originalName: file.originalname });
            });
        }
        if (req.files['liftingInvoiceFile']) {
            const file = req.files['liftingInvoiceFile'][0];
            data.liftingInvoice = { path: file.path.replace(/\\/g, "/"), originalName: file.originalname };
        }
...
app.put('/api/trucking/:id', cpUpload, async (req, res) => {
    try {
        ...
        const fields = ['carrier', 'customer', 'handler', 'booking', 'paymentStatus'];
        ...
        if (req.files['invoiceFiles']) {
            req.files['invoiceFiles'].forEach(file => {
                item.invoices.push({ path: file.path.replace(/\\/g, "/"), originalName: file.originalname });
            });
        }
        if (req.files['liftingInvoiceFile']) {
            if (item.liftingInvoice && item.liftingInvoice.path && fs.existsSync(item.liftingInvoice.path)) {
                fs.unlinkSync(item.liftingInvoice.path);
            }
            const file = req.files['liftingInvoiceFile'][0];
            item.liftingInvoice = { path: file.path.replace(/\\/g, "/"), originalName: file.originalname };
        }
...
app.delete('/api/trucking/:id', async (req, res) => {
    try {
        ...
        if (item) {
            if (item.invoices) {
                item.invoices.forEach(file => { try { if (fs.existsSync(file.path)) fs.unlinkSync(file.path); } catch (e) {} });
            }
            if (item.liftingInvoice && item.liftingInvoice.path) {
                try { if (fs.existsSync(item.liftingInvoice.path)) fs.unlinkSync(item.liftingInvoice.path); } catch (e) {}
            }
        }
...
app.post('/api/trucking/:id/delete-file', async (req, res) => {
    try {
        ...
        if (item) {
            ...
            if (item.liftingInvoice && item.liftingInvoice.path === filePath) item.liftingInvoice = undefined;
            await item.save();
        }
