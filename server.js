...
app.put('/api/trucking/:id', cpUpload, async (req, res) => {
    try {
        ...
        const fields = ['carrier', 'customer', 'handler', 'booking', 'paymentStatus', 'notes'];
        fields.forEach(f => item[f] = req.body[f]);
        if (req.body.containersList) item.containers = JSON.parse(req.body.containersList);
        ...
        if (req.files['liftingInvoiceFile']) {
            if (item.liftingInvoice && item.liftingInvoice.path && fs.existsSync(item.liftingInvoice.path)) fs.unlinkSync(item.liftingInvoice.path);
            const file = req.files['liftingInvoiceFile'][0];
            item.liftingInvoice = { path: file.path.replace(/\\/g, "/"), originalName: file.originalname };
        }
        await item.save(); res.json({ message: "Updated" });
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.delete('/api/trucking/:id', async (req, res) => {
    try {
        const item = await TruckingModel.findById(req.params.id);
        if (item) {
            if (item.invoices) item.invoices.forEach(file => { try { if (fs.existsSync(file.path)) fs.unlinkSync(file.path); } catch (e) {} });
            if (item.liftingInvoice && item.liftingInvoice.path) { try { if (fs.existsSync(item.liftingInvoice.path)) fs.unlinkSync(item.liftingInvoice.path); } catch (e) {} }
        }
        await TruckingModel.findByIdAndDelete(req.params.id); res.json({ message: "Deleted" });
    } catch (err) { res.status(500).json({ error: err.message }); }
});
